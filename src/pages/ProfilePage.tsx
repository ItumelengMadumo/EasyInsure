...
type Props = { portfolio: Portfolio; role: string; mode: 'live' | 'simulation'; capabilities: PersonaCapabilities; refresh: () => Promise<void>; notify: (message: string) => void };
export function ProfilePage({ portfolio, role, mode, capabilities, refresh, notify }: Props) {
  ...
  async function closeAccount() {
    ...
    const result = await client.mutations.requestAccountClosure({ idempotencyKey: crypto.randomUUID(), correlationId: crypto.randomUUID() });
    if (result.errors?.length) return notify(result.errors[0].message);
    if (mode === 'live') { await deleteUser(); window.location.assign('/'); }
    else { notify('Simulated account closure requested. Reset the scenario to restore the account.'); await refresh(); }
  }
  return <>
